# MD4Collision_Wang.py (updated @eliainnocenti)

#encoding: utf-8
import random
import struct
import time, re
from Crypto.Hash import MD4 # @eliainnocenti

#4字节
def Endian(b):
  # print(b[0:4])  # Optionally comment out noisy debug
...
#计算MD4
def MD4_hash(m):       # @eliainnocenti
  h = MD4.new()        # @eliainnocenti
  h.update(m)          # @eliainnocenti
  return h.hexdigest() # @eliainnocenti
...
def FindCollision(m_block: bytes):        # @eliainnocenti
  # m_block is now a 64‑byte bytes object # @eliainnocenti
  x = Endian(m_block) # 小端序
  ...
  # Pack to bytes                   # @eliainnocenti
  m1 = struct.pack('<16I', *x)      # @eliainnocenti
  m2 = CreateCollision(m1) #碰撞微分 # @eliainnocenti

  if MD4_hash(m1) == MD4_hash(m2):  # @eliainnocenti
    return m1, m2
  
  return None, None

#对于修正后的M，有一定概率可以通过碰撞微分找到M'
def CreateCollision(m_block: bytes) -> bytes:            # @eliainnocenti
    W = Endian(m_block)                                  # @eliainnocenti
    # apply your bit‑flips:                              # @eliainnocenti
    W[1] = (W[1] + (1 << 31)) & 0xFFFFFFFF               # @eliainnocenti
    W[2] = (W[2] + ((1 << 31) - (1 << 28))) & 0xFFFFFFFF # @eliainnocenti
    W[12] = (W[12] - (1 << 16)) & 0xFFFFFFFF             # @eliainnocenti
    # repack to bytes                                    # @eliainnocenti
    return struct.pack('<16I', *W)                       # @eliainnocenti

def Collision(): # @eliainnocenti (the whole function)
    """
    Search for an MD4 collision:
      • seed a random 64-byte block
      • call FindCollision(block) until it returns (m1, m2)
    Returns raw bytes for m1/m2 plus their common MD4 hex digest.
    """
    import sys
    from Crypto.Hash import MD4

    sys.stderr.write("[+] Finding Collision...\n")
    while True:
        seed = bytes(random.getrandbits(8) for _ in range(64))
        m1, m2 = FindCollision(seed)
        if m1 is not None:
            break
    ...
    h.update(m1)
    digest = h.hexdigest()
    return m1, m2, digest, digest

# Add Python 3–compatible main section for hex output
if __name__ == "__main__": # @eliainnocenti
